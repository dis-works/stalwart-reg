...
/// Generates a passphrase from EFF word list with `words` count
pub fn generate_passphrase(words: usize, delimiter: char) -> String {
    let mut buf = String::new();
    for _ in 0..words {
        if delimiter as u32 != 0 && !buf.is_empty() {
            buf.push(delimiter);
        }
        let rolls = roll_dice_5_times();
        let word = LIST_EFF.get(rolls.as_str()).unwrap_or(&"");
        buf.push_str(*word);
    }
    buf
}
